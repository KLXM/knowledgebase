...
final class FrontendRenderer
{
    ...
    public static function resolveChapterRenderAnchor(string $primaryValue, string $fallbackValue = ''): string
    {
        ...
        if ($isAutoGenerated && !str_starts_with($base, 'kb-chapter-')) {
            $base = 'kb-chapter-' . $base;
        }
        ...
        foreach ($articles as $article) {
            ...
            if (count($chapters) > 0) {
                ...
                foreach ($chapters as $chapter) {
                    ...
                    $items .= self::renderNavBadge($chapter['badge'], null, $chapter['badge_mode']);
        ...
        foreach ($articles as $article) {
            ...
            if (count($chapters) === 0) {
                ...
                foreach ($chapters as $chapter) {
                    ...
                    $groupItems .= self::renderNavBadge($chapter['badge'], null, $chapter['badge_mode']);
    ...
    * @param list<array{id:int,title:string,nav_title:string,slug:string,intro:string,excerpt:string,is_recent:bool,anchor:string}> $results
    ...
    /**
    * @return list<array{title:string,badge:string,badge_mode:string,anchor:string,level:string}>
     */
    private static function extractArticleChapters(\rex_data_knowledgebase_article $article, bool $topLevelOnly = true): array
    {
        ...
        foreach ($decoded as $slice) {
            ...
            $badgeData = self::resolveChapterBadge($data);
     ...
     * @return array{mode:string,badge:string}
     */
    private static function resolveChapterBadge(array $data): array
    {
        $mode = strtolower(trim((string) ($data['badge_mode'] ?? '')));
        $legacyBadge = trim((string) ($data['badge'] ?? ''));
        ...
        if ($mode === 'icon') {
            $value = $iconBadge !== '' ? $iconBadge : $legacyBadge;
            return ['mode' => 'icon', 'badge' => $value];
        }

        return ['mode' => 'auto', 'badge' => $legacyBadge];
    }
    ...
    private static function renderNavBadge(string $badge, ?string $defaultIcon = null, ?string $mode = null): string
    {
        $trimmed = trim($badge);
        ...
        if ($normalizedMode === 'text') {
            ...
            return '<span class="kb-app__nav-badge">' . rex_escape($trimmed) . '</span>';
        }

        if ($normalizedMode === 'icon') {
            if ($trimmed === '') {
                return self::renderNavBadgeIcon($defaultIcon);
            }
